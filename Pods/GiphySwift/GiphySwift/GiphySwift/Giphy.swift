...
public struct Giphy {
    ...
    fileprivate static let queue = DispatchQueue(label: "com.giphyswift", qos: .userInteractive, attributes: .concurrent, autoreleaseFrequency: .inherit, target: nil)
    
    static fileprivate let _dateFormatter = DateFormatter()
    ...
    fileprivate(set) static var apiKey = ApiKey.publicKey.key
    ...
    static fileprivate func dataTask<T: Collection>(with endpoint: GiphyRequestable, options: [String:String]? = nil, completionHandler: @escaping (GiphyResult<T>) -> Void) where T.Iterator.Element: GiphyModelRequestable {
    ...
    static fileprivate func jsonArray(from json: AnyObject?) -> [JSON]? {
        var jsonArray: [JSON]?
    ...
    static fileprivate func url(_ url: URL, with options: [String:String]?) -> URL? {
