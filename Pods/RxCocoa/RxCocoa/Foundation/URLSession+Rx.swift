...
extension Reactive where Base: URLSession {
    ...
    */
    public func response(_ request: URLRequest) -> Observable<(HTTPURLResponse, Data)> {
    ...
    */
    public func data(_ request: URLRequest) -> Observable<Data> {
        return response(request).map { (response, data) -> Data in
            if 200 ..< 300 ~= response.statusCode {
                return data
            }
            else {
    ...
    */
    public func json(_ request: URLRequest, options: JSONSerialization.ReadingOptions = []) -> Observable<Any> {
